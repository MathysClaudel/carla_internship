...
ARayCastCompleteLidar::FCompleteLidarDetection ARayCastCompleteLidar::ComputeDetection(const FHitResult& HitInfo, const FTransform& SensorTransf) const{
  // 1) On crée la struct qui contiendra point et intensité
  FCompleteLidarDetection Detection;
...
void ARayCastCompleteLidar::ComputeAndSaveDetections(const FTransform& SensorTransform)
{
    ...
    for (int32 id : Description.IgnoredActorIds) {
      // Récupérer le AActor* correspondant à cet id CARLA
      if (auto view = GetEpisode().GetActorRegistry().FindCarlaActor(id)) {
        const AActor *constActor = view->GetActor();
        AActor *actorPtr = const_cast<AActor*>(constActor);
        TraceParams.AddIgnoredActor(actorPtr);
      }
    }
