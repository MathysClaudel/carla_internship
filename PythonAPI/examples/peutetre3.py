...
def main():
    ...
    # 2) Attendre que CARLA soit prêt
    while True:
        try:
    ...
    settings.fixed_delta_seconds = 0.001 #a chaque appel de world.tick(), la simu carla avance de 0.001sec
    world.apply_settings(settings)
    print(f"Synchronous @ {1/settings.fixed_delta_seconds:.0f} Hz")

    # 4) Spawn véhicule à la position donnée et activer autopilot
    blueprint_library = world.get_blueprint_library()
    car_bp      = blueprint_library.find('vehicle.tesla.model3')
    spawn_point = carla.Transform(carla.Location(x=83.1, y=13.4, z=2.0))
    vehicle     = world.spawn_actor(car_bp, spawn_point)
    vehicle.set_autopilot(False)
    # Calcul de la vitesse cible : 40 km/h → m/s
    speed_m_s = 40.0 / 3.6
    # vecteur avant du véhicule
    fv = vehicle.get_transform().get_forward_vector()
    target_velocity = carla.Vector3D(fv.x * speed_m_s,
                                     fv.y * speed_m_s,
                                     fv.z * speed_m_s)
    # on applique cette vitesse en continu
    vehicle.set_target_velocity(target_velocity)
    print(f"Spawned vehicle at {spawn_point.location}, autopilot off")

    # 5) Warm-up : laisser tourner 5s pour que l’auto-pilot prenne bien la vitesse
    warmup_time  = 3.0
    warmup_ticks = int(warmup_time / settings.fixed_delta_seconds)
    print(f"Warming up for {warmup_time}s → {warmup_ticks} ticks")
    for _ in range(warmup_ticks):
        world.tick()

    # 6) Spawn LiDAR “anneau-par-tick”
    ...
    lidar_bp.set_attribute('sensor_tick',        '0.001') #meme que delta_time 
    ...
    # 7) Enregistrement pendant 5 s simulées
    sim_time  = 5.0
    num_ticks = int(sim_time / settings.fixed_delta_seconds)
    save_dir  = os.path.expanduser("~/lidar_with_timestamps_world_2")
