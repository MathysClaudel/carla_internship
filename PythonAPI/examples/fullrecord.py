...
class PointCloud:
    def __init__(self):
        ...
        self._semantic  = np.empty((0,3), dtype=np.float32)
        #self._normal  = np.empty((0,3), dtype=np.float32)

    def add_points(self, xyz, intensity, timestamp, semantic): #a rajouter :, normal
        ...
        self._semantic       = np.concatenate((self._semantic,    semantic.astype(np.float32)))
        #self._normal    = np.concatenate((self._normal,    normal.astype(np.float32) ))

    def save(self, path):
        # Prépare un array structuré pour PlyElement
        n_xyz = self._xyz.shape[0]
        n_seman  = self._semantic.shape[0]
        #n_normal = self._normal.shape[0]
        n = min(n_xyz, n_seman) # a rajouter : , n_normal
        ...
        xyz_slice       = self._xyz[:n]
        intensity_slice = self._intensity[:n]
        time_slice      = self._time[:n]
        sem_slice       = self._semantic[:n]
        #norm_slice      = self._normal[:n]
        vertices = np.empty(n, dtype=vertex_dtype)
        vertices['x']         = xyz_slice[:,0]
        vertices['y']         = xyz_slice[:,1]
        vertices['z']         = xyz_slice[:,2]
        vertices['intensity'] = intensity_slice
        vertices['time']      = time_slice
        vertices['cosine']         = sem_slice[:,0]
        vertices['id']         = sem_slice[:,1]
        vertices['tag']         = sem_slice[:,2]
        # vertices['nx']         = norm_slice[:,0]
        # vertices['ny']         = norm_slice[:,1]
        # vertices['nz']         = norm_slice[:,2]
...
def main():
    ...
    lidar_semantic_bp = blueprint_library.find('sensor.lidar.ray_cast_semantic')
    lidar_semantic_bp.set_attribute('range',              '100')
    lidar_semantic_bp.set_attribute('rotation_frequency', str(rotation_frequency))     # 10 révolutions/s
    lidar_semantic_bp.set_attribute('channels',           '32')     # 32 
    lidar_semantic_bp.set_attribute('points_per_second',  '640000') # densité
    lidar_semantic_bp.set_attribute('sensor_tick',        str(sensor_tick))  # 1 ms simulé = 1 tick
    lidar_semantic_bp.set_attribute('upper_fov',           '15')
    lidar_semantic_bp.set_attribute('lower_fov',           '-15')

    lidar_semantic = world.spawn_actor(
        lidar_semantic_bp,
        carla.Transform(carla.Location(z=2.0)),
        attach_to=vehicle
    )
    ...
    # lidar_normal_bp = blueprint_library.find('sensor.lidar.ray_cast_surface_normals')
    # lidar_normal_bp.set_attribute('range',              '100')
    # lidar_normal_bp.set_attribute('rotation_frequency', str(rotation_frequency))     # 10 révolutions/s
    # lidar_normal_bp.set_attribute('channels',           '32')     # 32 
    # lidar_normal_bp.set_attribute('points_per_second',  '640000') # densité
    # lidar_normal_bp.set_attribute('sensor_tick',        str(sensor_tick))  # 1 ms simulé = 1 tick
    # lidar_normal_bp.set_attribute('upper_fov',           '15')
    # lidar_normal_bp.set_attribute('lower_fov',           '-15')

    # lidar_normal = world.spawn_actor(
    #     lidar_normal_bp,
    #     carla.Transform(carla.Location(z=2.0)),
    #     attach_to=vehicle
    # )
    # lidar_normal_queue = Queue()
    # lidar_normal.listen(lambda data: lidar_normal_queue.put(data))
    # print("LiDAR attached and listening…")
    ...
    save_dir1  = os.path.expanduser("~/Data_Mathys_full/egomotion_semantic") #achanger
    ...
    n=k-1
    rev_counter=0
    print(f"nb de tick pour 1 frames complete: {k}")
    ...
    for _ in range(num_ticks):
        ...
        meas2 = lidar_semantic_queue.get()
        #meas3 = lidar_normal_queue.get()
        arr  = np.frombuffer(meas.raw_data, dtype=np.float32).reshape(-1,4)
        arr2  = np.frombuffer(meas2.raw_data, dtype=np.float32).reshape(-1,6)
        #arr3  = np.frombuffer(meas3.raw_data, dtype=np.float32).reshape(-1,7)
        ts   = meas.timestamp
        semantic  = arr2[:, 3:]  # cosine,object_id,semantic_tag
        #normal    = arr3[:, 4:]  # nx,ny,nz

        #print(f"DEBUG – semantic.shape = {semantic.shape}")
        # print("DEBUG – semantic sample:\n", semantic[:5])
        # print(f"DEBUG – normal.shape   = {normal.shape}")
        # print("DEBUG – normal sample:\n", normal[:5])
        ...
        if n%k == k-1 :
            pc.add_points(arr[:,:3], arr[:,3], ts, semantic) #a rajouter :, normal
            filename = template1 % rev_counter
            rev_counter+= 1
            pc.save(filename)
            #print(f"numero n de frame: {n}")
